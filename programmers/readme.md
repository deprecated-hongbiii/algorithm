...
- 그냥 풀었는데 사실 카카오의 출제 의도는 비트 연산이라고 한다.
- 비트 연산에 대해 공부해야겠다.

### 문제 해설 (카카오)

이 문제는 비트 연산Bitwise Operation을 묻는 문제입니다. 이미 문제 예시에 2진수로 처리하는 힌트가 포함되어 있고, 둘 중 하나가 1일 경우에 벽 #이 생기기 때문에 OR로 처리하면 간단히 풀 수 있습니다.
...
- 스택/큐라는 생각은 딱히 안 들었고 그냥 배열을 이용해 간단하게 풀었다.
- 처음 풀 때 놓친 점은 배열의 마지막 값이랑만 크기 비교를 했었다는 것. 그게 아니라 제일 큰 값과 비교를 해야 했다. 그래서 max 라는 변수를 추가함

### 다른 풀이

- 풀이는 거의 같은데 answer 배열에 요소를 변경하거나 추가할 때 객체 프로퍼티처럼 사용한 방식이 신박했다.
...
- 효율적인 풀이 방법이 생각 안 나서 그냥 풀었더니 효율성 0점!
- 카카오의 해설을 봤다.
  - 우선 각 info가 포함될 수 있는 모든 조합을 구해서 분류를 해놓는다. 예를들어 info가 abcd라면 abcd, abc-, ab--, a---, -bcd, -bc-, -b--, ... 이렇게 16가지의 분류로 미리 나눠놓는 것
  - 점수 기준으로 정렬을 한 뒤 이진 탐색을 하는데, lower bound를 사용하면 된다고 한다.
- 해설을 봐도 분류를 하라는 게 어떻게 하라는 건지 이해가 안 됐다.

### 다른 사람의 풀이

- 분류하는 방식은, 우선 객체를 활용한다.
- 분류 기준이 되는 문자열을 키로, 해당 분류 기준에 맞는 점수들의 배열을 밸류로 한다.
- 분류 기준이 되는 문자열을 구하는 방식을 재귀함수로 구현한 점이 놀라웠다.
- 객체에 작업을 완료했으면, 각각의 배열을 정렬한다.
- 정렬된 배열은 lower bound를 이용해 조건을 만족하는 결과를 구한다.
- lower bound란 주어진 값 이상의 값을 가지는 최소 위치를 찾는 것이다.
...
- 가장 큰 실수는 문제를 제대로 읽지 않았던 것 때문에 발생했다.
- seller 배열에 같은 사람이 중복해서 나타날 수 있다는 조건이 있었는데 그 부분을 제대로 읽지 않았다.
- 중복 가능성을 인지하고 다시 풀었는데도 계속 틀렸다.
  - 단순히 중복해서 나올 수 있으니 amount를 모두 합쳐서 한번에 계산하면 되겠다고 생각했다. enroll 배열 돌면서 table 만들 때 중복해서 나오는 사람의 amount를 모두 합치면 되겠다고 생각했다.
  - 그래서 처음에는 `findIndex` 가 인덱스를 하나만 찾아내니까, for문을 돌려 모든 인덱스를 찾아서 총 amount를 계산해서 넣었다.
  - 그랬더니 뒷부분에 seller 배열을 도는 부분에서 문제가 생겼다. table에는 amount를 합쳐서 넣어놓고, seller 배열은 여전히 중복이 나타나는데 그냥 돌면서 amount를 중복해서 계산하고 있던 것이었다.
  - 그래서 이번엔 seller 배열을 Set에 한 번 넣었다가 다시 배열로 바꿔서 계산했다.
  - 그래도 여전히 오답이었다.
- 그 이유는, 만약 mary의 이익이 15원, 15원이라고 했을 때 문제에 나온대로 계산하면 13원 + 13원 = 26원 이어야 하는데, 30원으로 합쳐서 계산하면 27원이 되기 때문이다.
- 최종적으로, amount를 미리 계산해두지 않고 seller 배열을 돌며 그때그때 계산하는 것으로 바꿨더니 문제가 풀렸다.

### 풀이

- 언뜻 트리를 구현하고 시작해야 하나? 라는 생각이 들 수도 있지만 그럴 필요까지는 없었던 문제이다.
- enroll에는 민호(center)를 제외한 모든 멤버가 있으므로, enroll 배열을 돌면서 table을 하나 만들어준다. table은 객체이며, key로는 멤버 이름, value로는 또 다시 객체인데 그 겍체는 referral(추천인===부모)과 total(총 수익)을 가지고 있다.
- while문으로 부모 노드가 없을 때까지, 즉 center(민호)에 도달할 때까지 탐색을 할 것이므로 마지막에는 멤버 이름을 `'-'` 로, referral을 `null` 로 하여 추가해준다.
  ```js
  {
    john: { referral: '-', total: 360 },
    mary: { referral: '-', total: 1318 },
    edward: { referral: 'mary', total: 108 },
    sam: { referral: 'edward', total: 0 },
    emily: { referral: 'mary', total: 450 },
    jaimie: { referral: 'mary', total: 18 },
    tod: { referral: 'jaimie', total: 180 },
    young: { referral: 'edward', total: 1080 },
    '-': { referral: null, total: 0 }
  }
  ```
- seller 배열을 돌며 계산을 한다. 부모 노드가 있을 경우에는 while문을 돌며 수익을 분배한다. 이 부분을 계산하기 위해 while문 안에서 변수 값을 계속 바꿔준다. (let으로 선언한 변수 `benefit`, `currReferral`, `currSeller`)
- 마지막에는 enroll 멤버의 순서대로 출력해야 하므로 enroll을 map으로 돌며 table에서 해당 멤버의 total과 매핑해준다.
